...
//
// Related Topics 哈希表 链表 双指针 👍 2972 👎 0
...
public class IntersectionOfTwoLinkedLists {
    ...
    public class Solution {
        // 使用双指针拼接路径。
        // 两个指针分别从两个链表头出发，当一个指针走到尽头时，就跳到另一个链表的头部继续走。
        // 这样两者走过的总路程都是 $m + n$，从而抵消了长度差，最终一定会同时到达交点（或同时到达 null）。
        public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
            ...
            ListNode pA = headA;
            ListNode pB = headB;
            // 当 pA 和 pB 没有相遇时，继续循环
            // 如果相交，它们会在交点处相等跳出循环
            // 如果不相交，它们会走完 m+n 后同时变成 null，null == null 也会跳出循环
            while (pA != pB) {
                // pA 走一步，如果走到尽头，就转到链表 B 的头部
                pA = (pA == null) ? headB : pA.next;
                // pB 走一步，如果走到尽头，就转到链表 A 的头部
